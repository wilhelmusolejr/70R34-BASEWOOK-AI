...
const actionSchemas = {
  ...
  publish_post: {
    description:
      'Leaf: publish a new post (one or more images + caption) to the user\'s own profile timeline. Navigates to facebook.com/me, opens the composer, uploads images via setInputFiles on the dialog\'s hidden <input type="file">, types the caption into the Lexical editor, and clicks Post. Caption source is controlled by `captionSource`: "ai" (default) generates via Gemini + system_prompt_post.txt, "post" uses the picked entry\'s caption (user.posts[].caption). Explicit `caption` param always wins. When `imageUrls` is omitted, auto-injects a random pick from user.posts[]; the picked entry\'s `context` seeds `postContext` and its `caption` seeds `postCaption`.',
    params: {
      imageUrls: {
        type: 'array',
        default: [],
      ...
      caption: {
        ...
        default: '',
        description:
          'Static caption — overrides both AI generation and post-data caption. Use this to hardcode test text.',
      },
      captionSource: {
        type: 'string',
        default: 'ai',
        enum: ['ai', 'post'],
        description:
          '"ai" generates the caption with Gemini from userIdentity + postContext. "post" uses the picked entry\'s caption (user.posts[].caption, injected as postCaption). Ignored if `caption` is set explicitly.',
      },
      postCaption: {
        type: 'string',
        default: '',
        description:
          'Caption from the picked post entry — auto-injected from user.posts[].caption. Used only when captionSource="post". Don\'t set manually; use `caption` for explicit overrides.',
      },
      userIdentity: {
        type: 'string',
        default: '',
  ...
  visit_profile: {
    ...
    params: {
      ...
      country: {
        type: 'string',
        description:
          'User country code (auto-injected). Used by "sharers" pool to fetch country-specific URLs.',
      },
    },
    hasChildren: true,
  },
  ...
  search: {
    description:
      'Navigator: search Facebook. Provide explicit `query`, or auto-generate — mode="name" (random first + last from country-matched name pools), mode="news" (random region + news keyword, country-aware), mode="page" ("{category} in {city}" — city auto-injected from user.city), or mode="general" (organic everyday topics like recipes, travel, memes — country-aware). Country auto-injected from user.country — US pools for US accounts, Italian for IT. Optional `filter` clicks a results tab. Use child steps to act on the results.',
    params: {
      query: {
        type: 'string',
        default: '',
      ...
      country: {
        ...
        default: '',
        description:
          'Country code for locale-aware query generation (e.g. "US", "IT"). Auto-injected from user.country when omitted. Defaults to US.',
      },
    },
    hasChildren: true,
  },
  marketplace_location: {
    description:
      'Leaf: open Facebook Marketplace and update the location. mode="auto" (default) clicks the location grabber icon to use the browser\'s geolocation (from proxy IP). mode="manual" types the city name from the user record. Self-navigates to facebook.com/marketplace/. City and country are auto-injected from the user record.',
    params: {
      mode: {
        type: 'string',
        default: 'auto',
        enum: ['auto', 'manual'],
        description:
          'How to set the location. "auto" uses the browser geolocation (proxy IP). "manual" types the city name.',
      },
      city: {
        type: 'string',
        default: '',
        description:
          'Target city for manual mode (e.g. "Roma, Lazio"). Auto-injected from user.city when omitted.',
      },
      country: {
        type: 'string',
        default: '',
  ...
  outlook_login: {
    ...
    params: {
      ...
      password: {
        ...
        default: '',
        description:
          'Outlook/Microsoft account password. Auto-injected from user.emailPassword when omitted.',
      },
    },
    hasChildren: false,
  },
